...
jQuery(document).ready(function(){
		...
		var error = formValidate(jQuery(".sc_contact_form form"), {
			...
			rules: [{
				...
				mask: {
					...
					message: "That's not an e-mail..."
				}
			}, {
				field: "message",
				min_length: {
					value: 1,
					message: "You might want to say something better than that!:)"
				},
				max_length: {
					value: 200,
					message: "Not sure if I'm going to read such long messages."
				}
			}]
		});
		if (!error) {
			...
			var data = {
			 ...
			 jQuery.post("sendmail.php", data, userSubmitFormResponse, "text");
	...
	function userSubmitFormResponse(response) {
		...
		if (rez.error == "") {
			jQuery(".sc_contact_form .result").addClass("sc_infobox_style_success").html("Your message was sent to me successfully!");
			setTimeout("jQuery('.sc_contact_form .result').fadeOut(); jQuery('.sc_contact_form form').get(0).reset();", 3000);
		} else {
			jQuery(".sc_contact_form .result").addClass("sc_infobox_style_error").html("Did something wrong with this e-mail sender! " + rez.error);
		}
